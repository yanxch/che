...
public class UserProfileJpaDaoTest {
    ...
    public void shouldBeAbleToSaveAndGet() throws Exception {
        //given
        Profile expected =
                new Profile("id2222").withUserId("user234234").withAttributes(ImmutableMap.of("key1", "value2", "key3", "value4"));
    ...
    public void shouldBeAbleToSaveAndGetMultiple() throws Exception {
        //given
        Profile p1 = new Profile("id11").withUserId("user1").withAttributes(ImmutableMap.of("key1", "value2", "key3", "value4"));
        Profile p2 = new Profile("id22").withUserId("user2").withAttributes(ImmutableMap.of("key12", "value22", "key32", "value42"));
        UserProfileDao dao = new UserProfileJpaDao(factory);
        //when
        dao.create(p1);
        dao.create(p2);
    ...
    public void shouldBeAbleToUpdateExisted() throws Exception {
        ...
        UserProfileDao dao = new UserProfileJpaDao(factory);
        dao.create(expected);
        expected = new Profile("id2222").withUserId("user234234").withAttributes(ImmutableMap.of("key1", "v33", "key4", "value5"));
        //when
        dao.update(expected);
        Profile actual = dao.getById("id2222");
        //then
