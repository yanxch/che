...
public class DebuggerPresenterTest extends BaseTest {
    ...
    @Mock
    private Promise<Value>  promiseValue;
    @Mock
    ...
    @Captor
    private ArgumentCaptor<Operation<Value>>        operationValueCaptor;
    ...
    public void testOnExpandVariablesTree() throws OperationException {
        ...
        doReturn(promiseValue).when(debugger).getValue(variable);
        doReturn(promiseValue).when(promiseValue).then((Operation<Value>)any());

        presenter.onExpandVariablesTree();

        verify(promiseValue).then(operationValueCaptor.capture());
        operationValueCaptor.getValue().apply(value);
    ...
    public void testOnBreakpointStopped() {
        ...
        verify(executionPoint).withTarget(eq(className));
