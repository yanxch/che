...
public class DebuggerPresenter extends BasePresenter implements DebuggerView.ActionDelegate,
    ...
    public void onExpandVariablesTree() {
        ...
        if (rootVariables.isEmpty()) {
            ...
            if (debugger != null) {
                Promise<Value> promise = debugger.getValue(selectedVariable.getVariable());

                promise.then(new Operation<Value>() {
                    @Override
                    public void apply(Value arg) throws OperationException {
                        selectedVariable.getVariable().setValue(arg.getValue());
                        List<DebuggerVariable> debuggerVariables = getDebuggerVariables(arg.getVariables());
        ...
        if (debugger != null && executionPoint != null) {
            ...
            promise.then(new Operation<StackFrameDump>() {
                ...
                public void apply(StackFrameDump arg) throws OperationException {
                    ...
                    variables.addAll(arg.getLocalVariables());
    ...
    public void onBreakpointStopped(String filePath, String className, int lineNumber) {
        ...
        executionPoint.withTarget(className);
