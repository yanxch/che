...
public class JavaPackageConnector implements NodeInterceptor {
    ...
    @Inject
    public JavaPackageConnector(PromiseProvider promises,
    ...
    protected Promise<List<Node>> transform(ResourceNode parent, final List<Node> children) {
        ...
        if (children.size() == 1) {
            ...
            if (node instanceof ContainerNode) {
                return getNonEmptyPackage(((ContainerNode)node).getData()).then(new Function<Container, List<Node>>() {
                    @Override
                    public List<Node> apply(Container pkg) throws FunctionException {
                        final Node packageNode = nodeFactory.newPackage(pkg, settingsProvider.getSettings());
            ...
            for (int i = 0; i < children.size(); i++) {
                ...
                if (node instanceof ContainerNode) {
                    ...
                    pkgPromises[pkgIndex] = getNonEmptyPackage(((ContainerNode)node).getData()).then(new Operation<Container>() {
                        @Override
                        public void apply(Container pkg) throws OperationException {
                            nodes[index] = nodeFactory.newPackage(pkg, settingsProvider.getSettings());
                        }
                    });
                } else {
                    nodes[index] = node;
            ...
            return promises.all(pkgPromises).then(new Function<JsArrayMixed, List<Node>>() {
                @Override
