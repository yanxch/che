...
public class JavaDebuggerUtils {
    ...
    public Location getLocation(com.sun.jdi.Location location) throws DebuggerException {
        String fqnLocation = location.declaringType().name();

        List<IType> types;
        try {
            Pair<char[][], char[][]> fqnPair = prepareFqnToSearch(fqnLocation);
            types = findByFqn(fqnPair.first, fqnPair.second);
        } catch (JavaModelException e) {
            throw new DebuggerException("Can't find class models by fqn: " + fqnLocation, e);
        }

        if (types.isEmpty()) {
            throw new DebuggerException("Type with fully qualified name: " + fqnLocation + " was not found");
        }
        ...
        if (type.isBinary()) {
            ...
            return new LocationImpl(type.getFullyQualifiedName(), location.lineNumber(), null, true, libId, projectPath);
        } else {
            ICompilationUnit compilationUnit = type.getCompilationUnit();
            String projectPath = type.getJavaProject().getPath().toOSString();
            String resourcePath = compilationUnit.getPath().toOSString();
            return new LocationImpl(type.getFullyQualifiedName(), location.lineNumber(), resourcePath, false, -1, projectPath);
        }
    }
