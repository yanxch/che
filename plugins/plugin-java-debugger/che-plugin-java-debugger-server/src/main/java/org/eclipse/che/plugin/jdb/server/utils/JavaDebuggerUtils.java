...
public class JavaDebuggerUtils {
     ...
     * @param projectPath
     *         path to project which we are debugging now
    ...
    public String convertFullProjectPath(String projectPath) {
        String workspaceLocation = ResourcesPlugin.getPathToWorkspace();
        if (projectPath.startsWith(workspaceLocation)) {
            projectPath = projectPath.replaceFirst(workspaceLocation, "");
        }
        return projectPath;
    }
}
